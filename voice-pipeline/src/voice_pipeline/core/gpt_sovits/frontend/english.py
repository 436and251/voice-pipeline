...
def _import_g2p_en_without_downloads():
    """Import g2p_en while neutralizing its import-time download hooks."""
    find = nltk.data.find
    download = nltk.download

    def local_find(resource_name, *args, **kwargs):
        if resource_name in {
    ...
    def no_download(*args, **kwargs):
        raise RuntimeError("g2p_en attempted a runtime NLTK download")

    nltk.data.find = local_find
    nltk.download = no_download
    try:
        import g2p_en.g2p as module
    finally:
        nltk.data.find = find
...
_g2p_en = _import_g2p_en_without_downloads()
G2p = _g2p_en.G2p
...
RESOURCE_DIR = os.path.join(current_file_path, "resources")
CMU_DICT_PATH = os.path.join(RESOURCE_DIR, "cmudict.rep")
CMU_DICT_FAST_PATH = os.path.join(RESOURCE_DIR, "cmudict-fast.rep")
CMU_DICT_HOT_PATH = os.path.join(RESOURCE_DIR, "engdict-hot.rep")
CACHE_PATH = os.path.join(RESOURCE_DIR, "engdict_cache.pickle")
NAMECACHE_PATH = os.path.join(RESOURCE_DIR, "namedict_cache.pickle")
...
class en_G2p(G2p):
    def __init__(self, tagger):
        ...
        cmudict = _g2p_en.cmudict
        _g2p_en.cmudict = EmptyCmudict()
        try:
            super().__init__()
        finally:
            _g2p_en.cmudict = cmudict
        self._tagger = tagger
        # 分词初始化
        wordsegment.load()
...
def g2p(text, engine):
    ...
    phone_list = engine(text)
    phones = [ph if ph != "<unk>" else "UNK" for ph in phone_list if ph not in [" ", "<pad>", "UW", "</s>", "<s>"]]

    return replace_phs(phones)


def _load_tagger(nltk_data_path: Path):
    original_paths = nltk.data.path[:]
...
class EnglishFrontend:
    def __init__(self, nltk_data_path: str | Path) -> None:
        path = Path(nltk_data_path)
        if not path.is_dir():
            raise FileNotFoundError(f"English NLTK data directory not found: {path}")
        self._engine = en_G2p(_load_tagger(path))

    def process(self, text: str) -> tuple[str, list[str], None]:
        normalized = text_normalize(text)
        return normalized, g2p(normalized, self._engine), None
